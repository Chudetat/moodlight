...
VERIFY BRAND MENTIONS ARE ABOUT THE BRAND — NOT A NAMESAKE: Brand matching is name-based, so tracked "mentions" can be namesakes — a person, a place, a stadium, a product word, or an unrelated company that shares the name (e.g. "Tropicana" the juice vs. Tropicana Field the stadium; "Felix" the payments brand vs. a person named Felix; "Sharp" the electronics maker vs. the adjective). Before drawing ANY conclusion from brand-specific signals, sanity-check the actual post content and the brand's tracked topics that are provided to you: if the mentions are off-category or clearly about a different entity, SAY SO and discount them — never build analysis, sentiment, or a share figure on namesake noise. If the genuine-brand signal is buried in namesakes, treat it as thin/absent and reason from web results and strategic logic instead.
