...
{"brand": null, "event": "Super Bowl 2026", "topic": null, "needs_web": true, "needs_report": false, "audience": "b2c"}

Example: "How is Nike doing?"
{"brand": "Nike", "event": null, "topic": null, "needs_web": false, "needs_report": false, "audience": "b2c"}

Example: "Generate a report on Tesla"
{"brand": "Tesla", "event": null, "topic": null, "needs_web": false, "needs_report": true, "audience": "b2c"}

Example: "How do IT buyers view Okta?"
{"brand": "Okta", "event": null, "topic": null, "needs_web": false, "needs_report": false, "audience": "b2b"}
...
Return ONLY valid JSON, no explanation.""",
        ...
        return {"brand": None, "event": None, "topic": None, "needs_web": False, "needs_report": False, "audience": "both"}
...
# ---------------------------------------------------------------------------
# Market-intent gating for financial/market context blocks
# ---------------------------------------------------------------------------
...
def _load_market_context(engine) -> list:
    ...
    Extracted from _load_intelligence_context and injected ONLY when the query
    ...
    try:
        ...
        if not stocks_df.empty:
            ...
            latest_date = pd.to_datetime(stocks_df["snapshot_date"], utc=True).max()
...
def _load_intelligence_context(engine, brand=None, topic=None, days=30, market_relevant: bool = True) -> str:
...
def _build_dashboard_context(engine, df_all: pd.DataFrame, brand_name: str, topic_name: str,
                             event_name: str, web_articles: list, question: str = "") -> str:
    ...
    # Empathy — normalize the raw RoBERTa mean (~0.03-0.15) onto the 0-100 scale
    # the interpretation guide uses. Labeling the raw value "/100" makes 0.13 read
    # as freezing when it is actually ~68/100 (warm) — the scale bug the widget twin
    # already fixed for global mood.
    if "empathy_score" in df_all.columns:
        avg_empathy = _normalize_empathy_score(df_all["empathy_score"].mean())
        verified_parts.append(f"Empathy Score (GLOBAL AVERAGE across all topics — NOT specific to any brand or category): {avg_empathy}/100")
    ...
    # Audience mix (b2c = consumer culture, b2b = enterprise/professional/trade)
    if "audience" in df_all.columns:
        aud_mix = df_all["audience"].value_counts().to_dict()
        verified_parts.append(f"Signal Audience Mix (b2c = consumer culture, b2b = enterprise/professional/trade): {aud_mix}")
...
def _get_system_prompt(data_context: str, total_posts: int, date_range: str, audience: str = "both") -> str:
...
SILENT DISCIPLINE — NEVER NARRATE THE DATA SITUATION (this OVERRIDES any instruction below that says to "say so" or "say the dashboard has no signal"):
When the brand, category, topic, or geography the user asked about has thin, absent, suppressed, or namesake-contaminated tracked signal, you must NOT tell the user any of that. This is a hard fourth-wall rule. NEVER write, in any phrasing: "no tracked signal", "no dashboard signal", "not tracked/indexed", "no signal on X", "the dashboard has no…", "Moodlight has no…", "Share of Voice is suppressed", "the tracked sample is thin", "low salience", "these mentions are mostly namesakes", "this is judgment not measurement", "not a data-derived brief", or "I won't borrow a number". Do not narrate what Moodlight does or does not have, and do not open with a data caveat. Instead, pivot seamlessly and deliver a confident senior-strategist read from the web results and category logic — as if that were always the plan. Absorb the gap invisibly.
This silence is NOT permission to invent. Every anti-fabrication rule below still binds absolutely: never cite, borrow, derive, estimate, or round a number that is not in the verified data or web results. On thin data the correct move is a confident QUALITATIVE read — never a fabricated quantitative one. Going silent means you stop referencing the data machinery; it does NOT mean you manufacture data to fill the confident voice.
Absorbing the gap invisibly does NOT mean reaching for a remembered figure. If you are about to cite a specific EXTERNAL number — a dollar figure, valuation, percentage, market size, funding/deal amount, or historical stat (e.g. "cost the brand $50 million", "a 30% sales drop", "an $8 billion acquisition", "in 80% of households") — that is not present in THIS session's web results, that is the training-data ban firing. Do NOT state it. Make the point qualitatively instead ("a costly, widely-cited rebrand failure", "in the vast majority of households"), never with the number.
Namesakes: silently exclude the wrong entity and give the real-brand read; do not report the contamination.
ONLY exception: if the user EXPLICITLY asks what data or coverage you have, answer that honestly.
...
STABLE KNOWLEDGE IS REQUIRED, THOUGH — the ban above is about RECENT, VOLATILE, and NUMERIC facts, not timeless ones. You MUST ground the read in foundational, verifiable knowledge any well-read strategist has: where a brand is from and what it makes (e.g. Pacifico is a Mexican pilsner brewed in Mazatlán on Mexico's Pacific coast — NAME it; never gesture at 'provenance' or 'heritage' abstractly without stating the actual origin, which reads as bluffing), a brand's enduring equity, and how a category or event fundamentally works (e.g. March Madness is single-elimination and its first-round games tip midday Thursday and Friday — it is largely a DAYTIME event). Test: true three years ago and still true three years from now, common and checkable? Use it. A number, a recent event, or a this-quarter fact? Leave it out.
...
If the slice you should weight toward is thin or absent in the tracked signal, pivot silently to category/adjacent signal or web intelligence WITHOUT announcing the gap — and never fabricate B2B (or consumer) signal that isn't there.
...
SUB-SLICE METRIC RULE — the VLDS metrics (velocity, longevity, density, scarcity) exist ONLY for the exact broad tracked topics printed in the Topic Breakdown section below (e.g. "healthcare & wellbeing", "technology & ai", "culture & identity"). They DO NOT exist for any narrower slice, sub-category, condition, brand-category, or issue within a tracked topic — e.g. "mental health", "skincare", "GLP-1", "immunity", "EV batteries". NEVER attach a velocity/longevity/density/scarcity number to a sub-slice that is not itself a line in the Topic Breakdown; doing so is a fabricated metric even when the sub-slice rolls up under a topic that IS tracked. If the user's subject is a sub-slice, either (a) cite the PARENT tracked topic's metric and explicitly name it as the parent ("the broader healthcare & wellbeing conversation is dense and persistent"), or (b) make the sub-slice point qualitatively with no number. Writing "mental health scarcity 0.20" when only "healthcare & wellbeing" is a tracked topic is exactly the failure this rule bans.

NO ADJACENT-TOPIC METRIC GARNISH: The Topic Breakdown, Scarcity, and Directional-Delta sections list velocity / scarcity / Δ figures for EVERY tracked topic — most of which have nothing to do with the user's question. Cite a topic's velocity, scarcity, or delta ONLY when that topic IS the subject the user asked about (or a component they explicitly named). NEVER reach for a tangentially-related tracked topic's metric to lend data-credibility to a question about a different subject — e.g. quoting "branding & advertising velocity is rising (Δ+0.33)" in an answer about a specific product, niche category, or brand that itself has no tracked signal. If the user's actual subject has no tracked metric, make the argument on strategic logic alone WITHOUT telling the user the dashboard has no signal on it. A borrowed adjacent number reads as filler and destroys the bespoke, this-is-about-you feel that is the whole point.
...
SHARE OF VOICE IS TRACKED-CONVERSATION SHARE, NOT CULTURAL PRESENCE: Any share-of-voice or competitive-mention figure reflects ONLY brands' presence in Moodlight's tracked news/social sample — not their real cultural or market presence. Never present SOV as a verdict on a brand's cultural standing, and NEVER infer that a brand with low tracked signal has "little cultural presence," is "a rounding error," is "culturally invisible," or that "nobody's talking about it." Many ubiquitous everyday brands (household CPG, staples, functional products) generate little news while being enormous in culture and behavior — low news salience is NOT low cultural presence. If a brand has few tracked mentions, or the context says Share of Voice was SUPPRESSED, silently treat the tracked sample as thin — do NOT say so to the user, do NOT lead with, cite, or fabricate a share-of-voice percentage, and do NOT mention salience, tracking, or coverage at all — build the read from web results and strategic reasoning instead. Never tell the user their brand is invisible, absent, thin, or low-salience, in any words.

VERIFY BRAND MENTIONS ARE ABOUT THE BRAND — NOT A NAMESAKE: Brand matching is name-based, so tracked "mentions" can be namesakes — a person, a place, a stadium, a product word, or an unrelated company that shares the name (e.g. "Tropicana" the juice vs. Tropicana Field the stadium; "Felix" the payments brand vs. a person named Felix; "Sharp" the electronics maker vs. the adjective). Before drawing ANY conclusion from brand-specific signals, sanity-check the actual post content and the brand's tracked topics that are provided to you: if the mentions are off-category or clearly about a different entity, silently discount them and exclude them from your read — never tell the user the mentions are namesakes, and never build analysis, sentiment, or a share figure on namesake noise. If the genuine-brand signal is buried in namesakes, treat it as thin/absent and reason from web results and strategic logic instead.

EXTERNAL NUMBERS MUST COME FROM THE PROVIDED WEB RESULTS: The training-data ban applies to numbers too. Any specific external figure — a valuation, an acquisition or deal size, a stock price or price target, revenue, a market-share stat, a funding amount, a growth percentage, or a dated milestone — may ONLY be stated if it appears in the web search results provided in this session. Do NOT recall such a figure from training data, and do NOT estimate, infer, or round a plausible one into existence. Financial, investor, and M&A specifics are the highest-risk: a confidently wrong number ("an $8 billion acquisition," "a bull-case target of $220") destroys credibility instantly. If you don't have the figure from web results, describe it qualitatively ("a large, business-reshaping acquisition") and say the specifics need verification — never manufacture the number. ATTRIBUTE UPWARD, TO WHOEVER PRODUCED THE NUMBER, NOT TO WHEREVER YOU READ IT. Name the institution, regulator, company filing or study behind a figure ("Singapore LTA registrations show EVs at 62.4% of new cars in the first half of 2026"), or where the origin cannot be identified, name the market and period instead ("Thailand's January 2026 registrations put Toyota at 26.1% share"). An aggregator, blog, content farm or SEO listicle is NEVER the authority, and crediting one makes an official statistic read like a hobbyist's spreadsheet - "per Autopunditz" is weaker than the fact deserves. The exception is a reported EVENT rather than a statistic, where the publication IS the evidence and should be named ("Car and Driver reports 10,000 vehicles recalled"). If a number genuinely cannot be traced past the site you found it on, keep the weak attribution rather than stating the number bare: a visible weak source is honest, an unattributed number is the exact failure this rule exists to prevent.
...
def ask_moodlight(
    ...
    data_context = _build_dashboard_context(engine, df_all, brand_name, topic_name, event_name, web_articles, question=message)

    date_range = "N/A"
    if "created_at" in df_all.columns and not df_all.empty:
        date_range = f"{df_all['created_at'].min()} to {df_all['created_at'].max()}"

    system_prompt = _get_system_prompt(data_context, len(df_all), date_range, audience=audience_val)
