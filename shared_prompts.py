...
KILL_CRITERIA = (
    ...
    "5. THE SUBSTITUTION TEST, THEN THE INEVITABILITY TEST. Both run on your sharpest claim, "
    "the one the whole answer rests on.\n"
    "   SUBSTITUTION: swap the subject for a different brand in the same category. If the "
    "sentence still works, it was never about this subject and it does not ship - rewrite "
    "until it only fits THIS one. 'Nike needs to reconnect with younger consumers' survives "
    "the swap and is therefore worthless. This is the single most reliable way to catch an "
    "answer that sounds intelligent and says nothing, and it is what separates a read from a "
    "competent summary of a category.\n"
    "   INEVITABILITY: once stated, does it feel like the only honest read of the evidence, or "
    "does it feel clever? Contrarian-for-its-own-sake fails. The best claims feel obvious in "
    "hindsight and were not obvious in advance. If the line feels forced it is a pose, and a "
    "pose is worse than a plain statement of what is true.\n\n"
    "6. WOULD A SENIOR PERSON SAY THIS OUT LOUD. Cut anything that is true but obvious, and "
