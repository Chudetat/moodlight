...
1b. AUDIENCE-AWARE READ. The signal is tagged b2c (consumer culture) vs b2b (enterprise/professional/trade) — consult the SIGNAL AUDIENCE MIX and, when present, the brand's "Audience Mix (b2c/b2b)" line BEFORE any invisibility call. If the brand has B2B signal, DELIVER THE B2B READ from it (IT/procurement/analyst/practitioner narrative) instead of declaring the brand invisible — a B2B brand absent from consumer culture but present in B2B signal is being measured by the wrong half of the instrument, not failing; serve the half that sees it. Only when the brand is thin in BOTH consumer AND b2b signal do you fall to the measurement-limit path in step 2.
