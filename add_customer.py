...
Usage:
  Add a customer:
    python add_customer.py add --email "jane@company.com" --name "Jane Smith" --tier foundation --credits 10
...
def add_customer(email, name=None, tier="foundation", credits=0):
...
def list_customers():
    ...
    for row in rows:
        ...
        credits_display = "unlimited" if tier == "enterprise" else str(credits)
...
def update_credits(email, add_credits):
    ...
    with engine.connect() as conn:
        ...
        if user[1] == "enterprise":
            print(f"ℹ️  {email} is on Enterprise tier (unlimited briefs). No credits needed.")
            return
...
def main():
    ...
    add_parser.add_argument("--tier", default="foundation", choices=["foundation", "enterprise"], help="Tier (default: foundation)")
